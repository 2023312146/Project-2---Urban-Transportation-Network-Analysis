...
class PathDisplay:
    ...
    def update_path_info(self):
        try:
            ...
            # 存储路径信息到主窗口
            if compare_result:
                # 使用get方法避免KeyError，设置默认值
                self.main_window.shortest_path = compare_result.get('dijkstra_path', [])
                self.main_window.efficiency_path = compare_result.get('efficiency_path', [])
            ...
            # 可选：记录错误日志或显示错误信息
            error_text = f"路径分析失败: {str(e)}"
            self.main_window.path_info.setText(error_text)
            self.main_window.path_info.setTextFormat(Qt.RichText)